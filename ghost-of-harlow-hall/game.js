...
const dom = {
  ...
  certificateCanvas: document.getElementById('certificate-canvas'),
...
Investigator: ${state.playerName || 'Unknown'}
Student ID: ${state.studentId || 'Unknown'}
...
const TUTORIAL_STEPS = [
  {
    icon: '🗂️',
    title: 'Your Mission',
    body: 'A student\'s identity has been stolen. You\'ve been assigned to find the perpetrator by collecting <strong>digital evidence</strong>, interviewing <strong>witnesses</strong>, and making a final accusation. Every decision matters.'
  },
  {
    icon: '📍',
    title: 'Navigate Locations',
    body: 'Use the <strong>Locations panel</strong> in the sidebar to move between investigation sites. Each location must be <strong>fully completed</strong> before the next one unlocks — collect all evidence and finish every interview before moving on.'
  },
  {
    icon: '🔎',
    title: 'Collect Evidence',
    body: 'Click <strong>highlighted objects</strong> in the scene to examine them. Read each piece carefully and click <strong>"Collect Evidence"</strong> in the pop-up to log it to your case file. Missing evidence affects your final score.'
  },
  {
    icon: '💬',
    title: 'Interview Witnesses',
    body: 'Click the <strong>Interview</strong> button at each location to question witnesses and suspects. Ask <strong>every available question</strong> — some answers unlock new leads and are required to complete the location.'
  },
  {
    icon: '⚖️',
    title: 'Ethics & Final Verdict',
    body: 'You\'ll face an <strong>ethics decision</strong> mid-investigation — handle it carefully, as it affects your score. When ready, head to the <strong>Security Office</strong> to submit your final verdict. Choose wisely: accusing the wrong person has consequences.'
  }
];
...
function renderTutorialStep() {
  ...
  nextBtn.textContent = tutorialStep === total - 1 ? 'Start Investigation' : 'Next';
}
...
function submitLogin() {
  ...
  const studentId = dom.loginStudentId.value.trim().toUpperCase();
  const idPattern = /^CS\d{8}$/;
  ...
  if (!idPattern.test(studentId)) {
    dom.loginError.textContent = 'Student ID must be CS followed by 8 digits (e.g. CS01083700).';
    return;
  }
...
function downloadCertificate() {
  ...
  const outcome = state.finalOutcome || calculateOutcome();
  const playerName = state.playerName || 'Investigator';
  ...
  ctx.fillText('UNIVERSITI TEKNOLOGI NUSANTARA', w / 2, 100);

  ctx.fillStyle = '#888';
  ctx.font = '11px "IBM Plex Mono", monospace';
  ctx.fillText('Digital Forensics Division', w / 2, 122);
  ...
  ctx.lineWidth = 1;
  ctx.beginPath();
  ctx.moveTo(w / 2 - 100, 205);
  ctx.lineTo(w / 2 + 100, 205);
  ctx.stroke();
  ...
  ctx.fillText(playerName, w / 2, 305);

  // Student ID
  ctx.fillStyle = '#c8a96e';
  ctx.font = '16px "IBM Plex Mono", monospace';
  ctx.fillText(studentId, w / 2, 340);

  // Completion text
  ctx.fillStyle = '#bbb';
  ctx.font = '14px "Georgia", serif';
  ctx.fillText('has successfully completed the digital forensic investigation', w / 2, 395);

  // Case name
  ctx.fillStyle = '#e8e0d0';
  ctx.font = 'bold italic 22px "Georgia", serif';
  ctx.fillText('"The Ghost of Harlow Hall"', w / 2, 435);

  // Case file
  ctx.fillStyle = '#888';
  ctx.font = '12px "IBM Plex Mono", monospace';
  ctx.fillText('Case File UTN-2024-0441', w / 2, 465);

  // Outcome
  ctx.fillStyle = outcome.correct ? (outcome.ethicalFlagged ? '#1d9e75' : '#c8a96e') : '#c0392b';
  ctx.font = 'bold 18px "Georgia", serif';
  ctx.fillText(outcome.title, w / 2, 520);

  // Score
  ctx.fillStyle = '#e8e0d0';
  ctx.font = 'bold 28px "Georgia", serif';
  ctx.fillText(`Score: ${outcome.score} / 100`, w / 2, 565);

  // Evidence and ethics
  ctx.fillStyle = '#888';
  ctx.font = '12px "IBM Plex Mono", monospace';
  ctx.fillText(`Evidence: ${outcome.evidenceCount}/12 collected  |  Ethics: ${outcome.ethicalFlagged ? 'Flagged' : 'Not flagged'}`, w / 2, 600);

  // Divider
  ctx.strokeStyle = 'rgba(200,169,110,0.3)';
  ctx.beginPath();
  ctx.moveTo(w / 2 - 200, 640);
  ctx.lineTo(w / 2 + 200, 640);
  ctx.stroke();

  // Date
  ctx.fillStyle = '#888';
  ctx.font = '12px "IBM Plex Mono", monospace';
  ctx.fillText(`Issued: ${new Date().toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}`, w / 2, 680);

  // Signature line
  ctx.strokeStyle = '#555';
  ctx.beginPath();
  ctx.moveTo(w / 2 - 100, 730);
  ctx.lineTo(w / 2 + 100, 730);
  ctx.stroke();
  ctx.fillStyle = '#888';
  ctx.font = '11px "IBM Plex Mono", monospace';
  ctx.fillText('Digital Forensics Division', w / 2, 750);

  // Download
  const link = document.createElement('a');
  link.download = `UTN_Certificate_${studentId}.png`;
...
function startGame() {
  const isNewGame = !state.evidence.length && state.currentScreen === 'title-screen';
  if (isNewGame) {
    state.currentLocation = 0;
  }
  loadLocation(state.currentLocation, { activateScreen: true });
  if (isNewGame && !state.tutorialSeen) {
    openTutorial();
  }
}
  ...
  document.addEventListener('keydown', event => {
    if (event.key === 'Escape') {
      if (document.getElementById('tutorial-overlay').classList.contains('open')) {
        skipTutorial();
      } else if (dom.modal.classList.contains('open')) {
        closeModal();
      }
    }
  });
...
function bootstrap() {
  ...
  if (saved) {
    ...
    if (!state.playerName || !state.studentId) {
      renderSidebar();
      renderJournal();
      showScreen('login-screen', { persist: false });
    } else {
